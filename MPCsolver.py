...
class MPCsolver(object):
	def __init__(self, num_steps, v_limit=1, a_limit=.1, phi_limit=.2, p_slack=1):
		...
		with tf.name_scope("constraints"):
			...
			self.a = tf.add(tf.multiply(tf.abs(self.t), tf.abs(self.phi)),tf.abs(self.t))
		...
		with tf.name_scope("loss"):
			...
			self.postion_loss = tf.reduce_mean(tf.subtract(self.target_distances, p_slack))

			self.loss = self.postion_loss + 2*self.velocity_loss + 2*self.phi_loss + 2*self.acc_loss - self.avg_v
